...
function compute(x, y, iterations, upper_bound, c){
    var real_part = x;
    var complex_part = y;
    var real_part_next;
    ...
    for(var i=1;i<=iterations;i++){
        real_part_next = (real_part*real_part - complex_part*complex_part) + c.real_part;
        complex_part_next = (2*real_part*complex_part) + c.complex_part;   //Z(n+1) = Z(n)*Z(n) + c
...
function plot(pixels, arg_width, arg_height, cartesian_bounds, iterations, upper_bound, c){
    console.log(cartesian_bounds);
    console.log(c);

    for(let x=0; x < arg_width; x++){
        for(let y=0; y < arg_height; y++){
                ...
                var color_val = this.compute(x_, y_, iterations, upper_bound, c)
...
self.onmessage = function(event){
    console.log("worker, message recieved, j.computed = " + event.data.computed);
    
    var j = event.data;

    j.pixels = plot(j.pixels, j.width, j.height, j.cartesian_bounds, j.iterations, j.upper_bound, j.c);
    
    j.computed = true;
    console.log("worker, computation done, sending message, j.computed = " + event.data.computed);
    postMessage(j);
}